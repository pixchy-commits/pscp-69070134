...
```text
ก้ข้อนี้ค่อนข้างง่ายครับ
จะใช้ loop ก็ได้ แต่ผมถนัด o(1) แบบ low memory 

basically ผมคิดแบบคณิตศาสตร์ครับ

แทนที่เราจะให้กบกระโดดทีละก้าวแล้วคอยนับ (Loop) เรามองมันเป็น สูตรผลรวมตัวเลข ทางคณิตศาสตร์ตั้งแต่แรก
ครั้งที่ 1 ได้  X 
ครั้งที่ 2 ได้ X - 2 
ครั้งที่ 3 ได้ X - 4

แค่นี้ก็เป็นไอเดียหลักแล้วครับบบ เดี๋ยวอธิบายต่อในวิธีทำ
```
...
แค่นี้ก็เป็นไอเดียหลักแล้วครับบบ 
such taht, ระยะรวม = n(X + 1) - n^2

ทีนี้ มันจะมี edge case คือเราต้องเช้คก่อนว่ากบมีแรงกระโดดถึงเป้าหมายไหม
เลยจะทำแบบนี้ครับ

กบจะหมดแรงเมื่อระยะกระโดดกลายเป็น 0 หรือติดลบ เราเลยหาจุดกระโดดได้มากที่สุดออกมาก่อน
จำนวนครั้งสูงสุดที่กระโดดได้คือ n_max = (X+1)/2 (ปัดลงด้วย)
เอาระยะทางสูงสุดที่กบทำได้มาเทียบกับ Y 
ถ้า ระยะสูงสุดยังน้อยกว่า Y ตอบ -1 ทันที

แล้วเราก็สามารถหา จำวนครั้งโดยการทำ algebraic solving ง่ายๆ
ก็แค่ย้ายข้างสมการ ก็จะได้ประมาณเป็น
...
```text
ผมก็อธิบายละเอียดมากๆแล้วแหละใน idea ในการแก้ปัญหาข้อนี้ในตอนต้นนน
ก็ใช้แผนนั้นเลยครับ
```
...
```text
impossible, whereas it's an edge case
6 + 4 + 2 = 12 m  but goal is 13m, therefore the program should return -1
```
