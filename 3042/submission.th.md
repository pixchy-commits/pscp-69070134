...
```text
basic เลยครับคือ รับ n มาเป็น input int

แล้วก็ให้ output เป็น string ที่ seperated ด้วย space
```
...
```text
ผมว่าจะ loop ตัั้งแต่ n จนถึง 0 เลย
แล้วก็ mod check ว่า n ตัวไหน % 10 แล้ว == 0

ถ้าหาก ระหว่างที่ i % 10 == 0
ดังนั้นเราจะก็ append I เข้า list เข้าไปเก็บไว้

แล้วค่อย print ออกมาตอนท้ายด้วย .join()
```
...
```text
เป็นแผนแรกที่คิดไว้เลยครับบบ แต่จริงๆ คิดว่าสามารถ optimize จาก o(n) เป็น o(1) ได้แต่ว่าแผนนี้ simple สุดและอ่านง่ายสุด ก็เลยใชเป็นแผนแรกที่เขียนไป
...
```text
เช็คว่า ระหว่างเลขจะไม่มีการ insert ซ้อน ซึ่งก็ควรไม่มีแหละ ฮ๋าๆๆๆ
```
...
```text
เคสที่ไม่สามารถหารอะไรได้เลย ควรจะได้ 0 ซึ่ง ต่ำ สุด
```
