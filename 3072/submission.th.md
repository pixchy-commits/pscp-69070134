...
```text
ไอเดียก็คือ เรา ได้ คำมา
แล้วต้องเก็บสระ แล้วนับสระออกมาว่า มีสระอะไรบ้าง ซึ่งต้องอย่ใน a e i o u 

และ จะต้อง count ออกมาเ ouput 
เช่น
a : 2
o : 1
```
...
```text
iterate  finding character in string using for loop
create an empty list of a voewl char
then checking if it's in the "aeiou" string. if yes, then append the empty list with the single char founded in the aeiou.

then another function outputting in this format using the same strategy
a : 2
o : 1
```
...
```text
used the one that i've mentioned earlier.
```
...
```text
basic of programmer, Hello World.
```
...
```text
check ว่า .lower ใช้ได้จริง และ ก้ คล้ายกับ test case 2 ที่ simulate ขึ้นมา
```
