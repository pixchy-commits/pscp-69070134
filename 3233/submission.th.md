...
ก็จะเป็น input ที่โจทย์ให้มา
ผมอ่านๆ ดู คร่าวๆ ก็น่าจะให้ตรวจสอบว่าเลข หลักตรงกันกี่หลัก
แล้วก็ใช้ if-else checking เยอะๆ แน่นอนเลย
```
...
1) ถูกครบทั้ง 5 หลัก (1,000,000 ถ้าอักษรตรง, 100,000 ถ้าอักษรไม่ตรง)

2) ถูกเลขท้าย 3 ตัว [-3:] (2,000 ถ้าอักษรตรง, 200 ถ้าอักษรไม่ตรง) 
แต่อย่าลืมว่า ต้องเช็ก 3 ตัวก่อน 2 ตัว เพราะหากถูก 3 ตัวจะถูก 2 ตัวด้วยเสมอ แต่มูลค่า 3 ตัวมากกว่า

3) ถูกเลขท้าย 2 ตัว [-2:] (1,000 ถ้าอักษรตรง, 100 ถ้าอักษรไม่ตรง)

4) หมวดอักษรตรงอย่างเดียว (20)

5) หากไม่ตรงเงื่อนไขใดเลย ให้แสดง 0

โค้ดก็จะหน้าตาประมารนี้
...
```text
ใช้ตาม intuitive core ideas ทีเกริ่นไปในตอนแรกเลยคัรบ แล้ว work ด้วย
...
```text
aksorn ไม่ตรง และถูกท้าย สามตัว
```
...
```text
ทดสอบเฉพาะ ตัวอักษร ตัวเลขบ่ถืกสักดอก
```
