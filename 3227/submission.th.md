...
```text
โจทย์จะให้ string มา แบ่งเป็น สองพาร์ท
พาร์ทแรกเป็น rank พาร์ทสองเป็น suit

เช่น QS, หรือ 10C 
เรามีหน้าที่แค่แปลง ให้เป้น queen of spades and 10 of clubs
```
...
```text
ไอเดียผม ผมเห้นว่าตัวสุดท้ายยังไงก็จะเป็น suite ผมก็เลยว่าจะใช้ string slice [-1] เพื่อรับ ตัวสุดท้ายมา
และก็ แรงก์ ก็จะมี ตัวเลข กับ ตัว อักษร

โดย ผมจะใช้ dictionary ในการเข้ามาช่วย
ผมจะสร้าง 2 dcitionary ขึ้นมาเป็น
...
หลังจากนั้น ผมก็จะ ใช้ .get() function ในการหาค่าที่ต้องการ

rank = rank_dict.get(rank_char, rank_char)
suit = suit_dict.get(suit_char, "")

โดย parameter ตัวสุดท้ายจะเป็น fallback value, in case ตอนที่ dictionary ไม่มี key อยู่ใน data structure
...
```text
ผมก็ใช้วิธีที่ผมอธิบายไปตอนเริ่มวางแผนเลยครับ ชิวๆ สบายมากๆๆ

รับ input มา
แปลงออกมาให้เป็น 2 part, rank and suite

เอา rank and suite เข้าหา ใน predefined dictionary เพื่อหา value ที่ตรง

แล้วก็ print ผลลัพธ์ออกมา
```
...
```text
เพราะมันเหมือน air conditioner หยอกเล่น 
usual test ธรรมดาเลยครับ
```
...
```text
เพราะมัน Hong kong หยอกเล่น ก็ usualy test
```
