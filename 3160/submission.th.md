...
```text
มีคนนั่งล้อมวง N คน (เริ่มที่คนที่ 1)
ส่งของข้ามไปทีละ K คน
ถ้าของวนไปถึงมือโจร (คนที่ T) โจรจะขโมยและวิ่งหนี = จบเกม
หรือถ้าของวนกลับมาที่คนที่ 1 อีกรอบ = จบเกม
```
...
```text
1: รับค่าตัวเลข 3 ตัว คือ N (จำนวนคนทั้งหมด), K (ระยะการส่งข้าม), T (ตำแหน่งของขโมย)
2: กำหนดค่าเริ่มต้น ให้ของอยู่ที่คนที่ 1 (current = 1) และนับจำนวนคนจับของขวัญเป็น 1 (count = 1)
3: ตรวจสอบกรณีพิเศษ ถ้าขโมยคือคนที่ 1 (T == 1) ให้พิมพ์ 1 แล้วจบการทำงานทันที
4: ถ้าไม่ใช่ ให้เข้าสู่ลูป (Loop) เพื่อจำลองการส่งของ
5: ในลูป หาตำแหน่งคนรับคนถัดไป โดยใช้การหารเอาเศษ (Modulo) เข้าช่วย
6: เช็คเงื่อนไขในลูป:

6.1: ถ้าคนถัดไปคือขโมย (current == T) ให้นับจำนวนคนเพิ่มอีก 1 แล้วหยุดลูป

6.2: ถ้าของวนกลับมาที่คนที่ 1 อีกครั้ง (current == 1) ให้หยุดลูป

6.3: ถ้าไม่เข้าเงื่อนไขเลย ให้นับจำนวนคนเพิ่มขึ้น 1 แล้ววนลูปส่งต่อ
7: พิมพ์จำนวนคนทั้งหมดที่ได้จับของขวัญ (count)
```
