...
```text
ข้อนี้ตอนแรกผมก็นึกว่าจะเป็นแบบ game theory + ใช้ DFS, BFS ไรงี้
แต่พอมานึกดูดีๆ จะขนาดนั้นจริงๆหรอ 5555
ก็เลยนั่งงมสักพักก็เจอ pattern ที่ว่า แต่ละชั้นมั้นก็จะมี pattern ตายตัวของมัน
โดยเฉพาะสามเหลี่ยมหงาย สามเหลี่ยมคว่ำ ที่ step ในการเดินจะไม่เหมือนกัน
ประมาณนี้ครับ
```
...
```text
ผมเห็นว่า ชั้นที่ n และ ลำดับ ที่ c และนับว่า เป็นคู่/คี่ มันจะมี step ในการเดินเท่ากัน

ง่ายๆ ผมก็เห้นว่า ขวามือสุดมันจะเป็น กำลังสอง ของชั้นที่ n
ผมก็จะ แทน r ว่าเป็นชั้น และ จะเท่ากับ math.ceil(math.sqrt(N)) ที่ต้องใช้ ceil ก็เพราะว่าสมมุติว่าอยู่ชั้นนั้นแต่ ไม่ใช่ perfect number ก็จะได้ return ว่าอยู่ชั้นนั้น ตรงกับ pattern

ต่อไป หาว่าอยู่ลำดับที่เท่าไหร่ ด้วย c = N - (r-1)^2
หมายความว่าก็แค่เอาเลขห้อง N ไปลบกับเลขห้องสุดท้ายของชั้นที่แล้ว ซึ่งก็คือ (r-1)^2

สุดท้ายก็แค่นับกำแพงจากทิศทางของห้อง คู่และคี่
หาก c เป็นคี่(สามเหลี่ยมชี้ขึ้น) หมายความว่าด้านบนมันตัน จำนวนกำแพงที่จะต้องพังก็เป็น 2r - 2

กลับกันหาก c เป็นคู่ ก็จะได้ เป็น 2r - 3 เพราะมันมีกำแพงด้านบน เพราะเป็นสามเหลี่ยมชี้ลงนั่นเอง ง่ายๆ ชิวๆ สบายบรื้อ สะดือโบ๋
```
...
```text
ใช้ตามแผนแรกที่กล่าวไปหมดเลย 555555 ไม่อยากพิมพ์ซ้ำอ่า
```
...
```text
เป็นจุด start point, so ต้องเท่ากับ 0 
```
...
```text
21 เดิมทีจะได้ 8 แต่ เพราะ 2r - 2
มาดู 22 ก็ควรจะได้ 7 เพราะ 2r -3
```
...
```text
เช็คว่าไม่ timeout เพราะ o(1) ถถถ ไม่ใช่อะไรนึก test case ไม่ออกก็เลย ใส่เลขมั่วๆไป 
```
