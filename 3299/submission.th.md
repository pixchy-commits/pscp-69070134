...
```text
แถบกว้าง L ปลูกไปทั้งหมด N ต้น
pattern มันผลรวมของอนุกรมเนอะ สามเหลี่ยมธรรมดา
ประมาณนี้เลยครับลองสังเกตดูก้ได้

แค่หาว่าปลูกไปถึง แถบที่เท่าไหร่ แล้ว sum มันถึงโควต้า N ต้นพอดี
```
...
```text
หา pattern ง่ายๆก็คือ จำนวนดอกไม้ที่ปลูกไปจนจบแถบที่ k มันคือสูตรผลรวมอนุกรม (k*L) * (k*L + 1) // 2 
เราก็แค่loopหาว่าแถบที่ k ไหนที่ผลรวมมัน able to reach N ก้จะได้สิ่งที่โจทย์ต้องการ

รับ str มาแล้ว split ได้ L กับ N 
ตั้งค่าเริ่มแถบที่ k = 1 
วนลูป while เช็คว่า (k*L)*(k*L+1)//2 < N ใช่ป่าว ถ้าน้อยกว่าก็บวก k += 1 ไปเรื่อยๆ จนกว่าผลรวมมันจะพอดีหรือเลย N
```
...
```text
ใช้ตามแผนแรกเป๊ะๆเลย mental model ก็ดีงี้แหละ :))
```
...
```text
edge เคส
ไม่ต้องเข้าลูปป และควรตอบทันที ซึ่งก็ตรง
```
...
the perfect
row 1 ปลูกได้ 3 
row 2 ปลูกรวมได้ 10 พอดี, so it's enough.
```
...
```text
จริงๆ แค่เช็คตาม constraint ซึ่งเรารุ้ว่าเราใช้ loop ดังนั้นมันมี Time out แน่นอน
ก้เลยเช็คตาม constraint โจทย์เฉยๆ ไม่มีอะไรมาก
```
